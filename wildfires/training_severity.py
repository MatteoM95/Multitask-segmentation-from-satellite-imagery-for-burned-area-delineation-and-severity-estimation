...
class Training_Severity:
    ...
    def start_initialize_and_train(self):
        ...
        self.val_dataset = DatasetCEMS_Severity(root=Path(self.cfg.rootData), annotation_type = self.cfg.annotation_type_severity, csv_satelliteData=Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_val), cache=False, severity_convention=self.cfg.severity_convention)
        self.test_dataset = DatasetCEMS_Severity(root=Path(self.cfg.rootData), annotation_type = self.cfg.annotation_type_severity, csv_satelliteData=Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_test), cache=False, severity_convention=self.cfg.severity_convention)
        ...
        # Paths
        self.model_name_folder = 'GRA_{}_{}_{}/'.format( self.args.segmentation_network, self.args.num_epochs * self.args.sample_per_epoch, str(self.args.date.strftime( '%m%d_%H' )) )
        self.model_name = 'GRA_{}_{}_{}.pth'.format( self.args.segmentation_network, self.args.num_epochs * self.args.sample_per_epoch, str(self.args.date.strftime( '%m%d_%H' )) )
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(self.val_dataloader):
                ...
                if not saved and setted:
                    ...
                    # print(f"FIRST label: {first_image.shape}")
                ...
                labels_delineation =  (torch.sigmoid(labels_severity) >= 0.20).int()
