...
from wildfires.dataset import DatasetCEMS_Multitask
...
from wildfires.utils import save_log2df, save_model_configuration, set_seed, save_configuration_tb, compute_confusion_matrix, \
                            compute_prec_recall_f1_acc, weight_init,  remap_severity_labels, to_RGB_Mask ,compute_squared_errors #, compute_iou,
...
class Training_Multitask:
    ...
    def start_initialize_and_train(self):
        ...
        elif self.args.segmentation_network == 'unetAtt':
        ...
        elif self.args.segmentation_network == 'unetplusAtt':
        ...
        self.train_transform = A.Compose([
                                ...
                                additional_targets={"delineation": "mask", "grading": "mask"},
                            )

        # Create datasets and dataloaders
        self.train_dataset = DatasetCEMS_Multitask(root=Path(self.cfg.rootData), annotation_type = self.cfg.annotation_type_severity,  csv_satelliteData=Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_train), transforms=self.train_transform, cache=False, severity_convention=self.cfg.severity_convention) # transforms=self.train_transform, 
        self.val_dataset = DatasetCEMS_Multitask(root=Path(self.cfg.rootData), annotation_type = self.cfg.annotation_type_severity, csv_satelliteData=Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_val), cache=False, severity_convention=self.cfg.severity_convention)
        self.test_dataset = DatasetCEMS_Multitask(root=Path(self.cfg.rootData), annotation_type = self.cfg.annotation_type_severity, csv_satelliteData=Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_test), cache=False, severity_convention=self.cfg.severity_convention)
        ...
        if self.args.loss == "bce":
            ...
            self.loss_criterion = DiceLoss(from_logits = True, ignore_index= 255, mode='binary', eps=0.000001)
        ...
        # Paths
        self.model_name_folder = 'MUL_{}_{}_{}/'.format( self.args.segmentation_network, self.args.num_epochs * self.args.sample_per_epoch, str(self.args.date.strftime( '%m%d_%H' )) )
        self.model_name = 'MUL_{}_{}_{}.pth'.format( self.args.segmentation_network, self.args.num_epochs * self.args.sample_per_epoch, str(self.args.date.strftime( '%m%d_%H' )) )
    ...
    def start_train(self):
        print("\n", "-" * 100, sep="")
        print(f"\nTRAINING model: \t{self.args.segmentation_network}  \n\n - TrainSet: \t\t{Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_train)}  \n - ValidationSet: \t{Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_val)}  \n - TestSet: \t\t{Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_test)}\n")
        print("-" * 100, "\n", sep="")
        ...
        for _ in range(epoch_number, self.args.num_epochs+1):
            print(' --- EPOCH {}'.format( epoch_number), end=" ")
            ...
            if epoch_number % self.args.checkpoint_step == 0 and epoch_number != 0:
                ...
                model_name_checkpoint = 'CP{}_GRA_{}_{}_{}.pth'.format( epoch_number, self.args.segmentation_network, self.args.num_epochs * self.args.sample_per_epoch, str(self.args.date.strftime( '%m%d_%H' )) )
        ...
        # save hyperparameters and configuration in tensorboard  test_IoU_delineation, test_f1_delineation, test_IoU_severity, test_f1_severity,
    ...
    def train_one_epoch(self, epoch_index, tb_writer):
        ...
        for i, batch in enumerate(self.train_dataloader):
            ...
            labels_delineation = batch["del_mask"].to(self.device)
            labels_severity = batch["gra_mask"].to(self.device)
 
            # count number pixel burned per batch
            for label_mask in labels_delineation:
            ...
            pixels_total += labels_delineation.numel()
            burned_pixels_batch = 0
        
            logits_delineation, logits_severity = self.model(images)

            loss_delineation = self.loss_criterion(logits_delineation, labels_delineation)
    ...
    def validate_model(self, epoch_number):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(self.val_dataloader):
                ...
                loss_delineation = self.loss_criterion(logits_delineation, labels_delineation)
                pred_delineation = (torch.sigmoid(logits_delineation) > 0.5).int()
