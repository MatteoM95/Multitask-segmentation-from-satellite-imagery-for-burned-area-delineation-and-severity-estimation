...
class Training_Delineation:
    ...
    def start_initialize_and_train(self):
        ...
        self.train_dataset = DatasetCEMS_Delineation(root=Path(self.cfg.rootData), annotation_type = self.cfg.annotation_type_delineation,  csv_satelliteData=Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_train), transforms=self.train_transform, cache=False) # transforms=self.train_transform, 
        self.val_dataset = DatasetCEMS_Delineation(root=Path(self.cfg.rootData), annotation_type = self.cfg.annotation_type_delineation, csv_satelliteData=Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_val), cache=False)
        self.test_dataset = DatasetCEMS_Delineation(root=Path(self.cfg.rootData), annotation_type = self.cfg.annotation_type_delineation, csv_satelliteData=Path(self.args.dataset_CSV_folder + self.cfg.dataset_path_test), cache=False)
        ...
        # Paths
        self.model_name_folder = 'DEL_{}_{}_{}/'.format( self.args.segmentation_network, self.args.num_epochs * self.args.sample_per_epoch, str(self.args.date.strftime( '%m%d_%H' )) )
        self.model_name = 'DEL_{}_{}_{}.pth'.format( self.args.segmentation_network, self.args.num_epochs * self.args.sample_per_epoch, str(self.args.date.strftime( '%m%d_%H' )))
        ...
        print(f'tb csv folder {self.model_runs_path}')
        print(f'tb path: {self.tensorboard_path}')
    ...
    def start_train(self):
        ...
        for _ in range(epoch_number, self.args.num_epochs+1):
            ...
            if epoch_number % self.args.checkpoint_step == 0 and epoch_number != 0:
                ...
                model_name_checkpoint = 'CP{}_{}.pth'.format( epoch_number, str(self.args.date.strftime( '%m%d_%H' )) )
